...
app.post('/createReservation', async (req, res) => {
  const { id_user, id_event, nama_tim } = req.body;
  console.log(id_user, id_event, nama_tim)
  try {
    ...
    res.status(201).json({ status: "success",  message: 'Reservasi berhasil dibuat!', reservation: savedReservation, reservationId: savedReservation._id });
  } catch (err) {
    res.status(500).json({ status: "failed", message: 'Gagal membuat reservasi: ' + err.message });
  }
});
...
app.post('/createPayment', async (req, res) => {
  ...
  try {
    ...
    const reservation = await Reservation.findById(id_reservasi).populate('id_event').populate('id_user');
    console.log("reservation", reservation)
    console.log("harga", reservation['id_event']['htm_event'])
    console.log("metode", metode_pembayaran)
    if (!reservation) {
    ...
    const parameter = {
      transaction_details: {
        order_id: `ORDER-${Date.now()}`, // ID unik untuk transaksi
        gross_amount: reservation['id_event']['htm_event'],
      },
      item_details: [
        {
          id: reservation['id_event']['_id'],
          price: reservation['id_event']['htm_event'],
          quantity: 1,
      ...
      customer_details: {
        ...
        email: reservation['id_user']['email_user'], // Ganti dengan email pengguna yang sesuai
      },
    };
...
// Contoh endpoint untuk menangani callback dari Midtrans
app.post('/midtrans-callback', async (req, res) => {
  const { order_id, transaction_status, fraud_status } = req.body;

  try {
    const reservation = await Reservation.findOne({ _id: order_id });
    if (!reservation) {
      return res.status(404).json({ message: 'Reservasi tidak ditemukan!' });
    }
    ...
    await reservation.save();
    res.status(200).json({ message: 'Status pembayaran diperbarui!' });
  } catch (err) {
    res.status(500).json({ message: 'Gagal memperbarui status pembayaran: ' + err.message });
  }
});
...
app.get('/getReservations/:id_user', async (req, res) => {
  ...
  try {
    const userReservations = await Reservation.find({ id_user }).populate('id_event').populate('id_user');
